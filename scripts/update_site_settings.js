...
async function updateSettings() {
  ...
  const { error: error1 } = await supabase
    .from("site_settings")
    .upsert({ key: "banner_title", value: "出海资源铺" }, { onConflict: "key" })
  
  if (error1) {
    console.error("更新 banner_title 失败:", error1)
  } else {
    console.log("banner_title 已更新为: 出海资源铺")
  }

  // 更新 banner_subtitle
  const { error: error2 } = await supabase
    .from("site_settings")
    .upsert({ key: "banner_subtitle", value: "一站式出海资源采购平台，助力全球化业务拓展" }, { onConflict: "key" })
  
  if (error2) {
    console.error("更新 banner_subtitle 失败:", error2)
  } else {
    console.log("banner_subtitle 已更新为: 一站式出海资源采购平台，助力全球化业务拓展")
  }
